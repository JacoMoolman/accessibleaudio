...
$publicProductionPages = "$index`n$audiobooks`n$contact`n$faq`n$terms`n$voiceSamples`n$submitIndex`n$submitApp"
...
Assert-Contains $index '<meta property="og:title" content="Accessible Audio | AI Audiobooks">' "homepage"
...
Assert-NotContains $audiobooks '<a href="submit/">Submit</a>' "audiobook library navigation"
Assert-NotContains $contact '<a href="submit/">Submit</a>' "contact navigation"
Assert-NotContains $faq '<a href="submit/">Submit</a>' "FAQ navigation"
Assert-NotContains $terms '<a href="submit/">Submit</a>' "terms navigation"
Assert-NotContains $voiceSamples '<a href="submit/">Submit</a>' "voice samples navigation"
Assert-NotContains $submitIndex '<a href="https://accessibleaudio.co.za/submit/"' "submit navigation"
...
Assert-Contains $index "styles.css?v=20260718-voicefit1" "homepage"
...
Assert-Contains $audiobooks "styles.css?v=20260718-voicefit1" "audiobook library"
Assert-Contains $audiobooks "scripts/site-motion.js?v=20260715-motion2" "audiobook library"
Assert-Contains $contact "styles.css?v=20260718-voicefit1" "contact page"
...
Assert-Contains $faq "styles.css?v=20260718-voicefit1" "FAQ page"
...
Assert-Contains $voiceSamples "styles.css?v=20260718-voicefit1" "voice samples page"
...
Assert-NotMatch "$voiceSamples`n$voiceSamplesScript`n$voiceCatalogScript" "(?i)gemini|google text|omnivoice|voice provider|local ai|cloud voices|local voices" "public voice sample interface"
Assert-Contains $voiceSamples 'id="voice-list"' "voice samples page"
Assert-Contains $voiceSamples "Available voices" "voice samples page"
...
Assert-Contains $styles "grid-template-columns: repeat(2, minmax(0, 1fr));" "stylesheet"
Assert-Contains $styles "background: var(--soft);" "stylesheet"
Assert-NotContains $styles "#fffdf7" "stylesheet"
Assert-Contains $voiceCatalogScript "const firstVoiceNumber = 1" "voice catalog script"
Assert-Contains $voiceCatalogScript "const voiceCount = 30" "voice catalog script"
Assert-Contains $voiceCatalogScript "const sampleFileOffset = 5" "voice catalog script"
Assert-Contains $voiceCatalogScript "const costPerWordCents = 0.75" "voice catalog script"
...
Assert-Contains $submitIndex "./styles.css?v=20260721-downloads1" "submit page"
Assert-Contains $submitIndex "./app.js?v=20260721-downloads1" "submit page"
...
Assert-Contains $submitIndex "Voice narration: 0.75c/word." "submit page"
...
Assert-Contains $submitApp 'formData.append("terms_version", "2026-07-18")' "submit app"
Assert-Contains $submitApp 'formData.append("output_format", "mp3")' "submit app"
Assert-NotContains $submitIndex 'name="output-format"' "submit page"
Assert-Contains $submitIndex "chapter-ready MP3 files" "submit page"
Assert-Contains $faq "Which audio format will I receive?" "faq page"
Assert-Contains $terms "chapter-ready <strong>MP3 files</strong>" "terms page"
...
Assert-Contains $adminApp '/api/admin-delete.php' "admin app"
Assert-Contains $adminApp 'data-delete-job' "admin app"
Assert-Contains $adminApp 'Delete book and audio' "admin app"
Assert-Contains $adminApp 'response.blob()' "admin app"
Assert-Contains $adminIndex 'id="audit-section"' "admin page"
Assert-Contains $adminIndex './app.js?v=20260721-audit1' "admin page"
Assert-Contains $adminIndex './styles.css?v=20260721-audit1' "admin page"
Assert-Contains $adminApp '/api/admin-audit.php?limit=200' "admin audit app"
Assert-Contains $adminApp 'recordAuditEvent("admin.login")' "admin audit app"
Assert-Contains $adminStyles '.audit-card' "admin audit stylesheet"
...
Assert-Contains $index "Accessible Audio | AI Audiobooks" "homepage"
Assert-Contains $index "AI audiobook production" "homepage"
Assert-Contains $index "AI-generated audiobook" "homepage"
Assert-Contains $index "Built for consistent, review-ready audiobook production." "homepage"
Assert-Contains $index "Consistent voice catalogue" "homepage"
Assert-NotMatch $publicProductionPages "(?i)locally produced|processed locally|handled locally|local production|local voices|cloud voices|automated grok|gemini text to speech" "public pages"
...
Assert-Contains $index "Structured AI processing" "homepage"
